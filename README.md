...
pnpm -r --filter vite-vue install @neopark/common // 因为他会从 远程npm仓库去找 @neopark/common 而不是从本地
...
5.packages 安装 子包
  ...
  2.pnpm install -r --filter @neopark/common  // 类似 npm init


6.构建build 公共资源
// 7+版本 先切换 再执行 scripts 命令; 前提是 @neopark/common(package/common) pkg.json 有 build指令
eg: pnpm --filter @neopark/common run build
...
7.工作区内 包与包之间引用 是通过 symbolic链接资源
    ...
    pnpm -r --filter @neopark/react // 安装依赖 -r: pnpm rebuild 重新build
...
先增加pkg workspace 依赖:
  "dependencies": {
    "@neopark/common": "workspace:*" // 硬链接.pnpm-store  vite-app node_modules下就会找到该依赖
  },
...
pnpm install @neopark/common  -w

2.pnpm -r --filter @neopark/common build 重新build完
